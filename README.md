...
- **三个可调参数**：灵敏度阈值、最小卡点间隔、全局时间偏移（毫秒，用来补偿音频延迟）。
...
| 参数 | 范围 | 默认 | 作用 | 怎么调 |
| --- | --- | --- | --- | --- |
| 灵敏度阈值 | 0 ~ 100 | 50 | 阈值越高，只有明显的重鼓点会被标记 | 卡点太碎/太多少 → 调大；漏掉轻鼓点 → 调小 |
| 最小卡点间隔 | ≥ 0 秒 | 0.15 | 两个卡点之间的最小间隔，防止同一个鼓点被重复标记 | 鼓点非常密时可调小到 0.1；只想留每拍 → 调到 0.5 左右 |
